...
// Added compatibility with Lazarus + merged with single gnu-gzip
...
uses
 Windows {$IFDEF UseDelphi}, Classes, Graphics, SysUtils{$ENDIF},SHL_GNU_gzip;
...
type
  ...
  public
    ...
    procedure {%H-}LoadFromResourceName(Instance: HInst; const Name: String);
    procedure {%H-}LoadFromResourceID(Instance: HInst; ResID: Integer);
    ...
    {$IFDEF FPC}procedure {%H-}SetTransparent(b:Boolean);{$ENDIF}
  end;
...
function inflateInit_(var strm: TZStreamRec; version: PChar;
  recsize: Integer): Integer;
begin
  Result:=SHL_GNU_gzip.inflateInit_(@strm,version,recsize);
end;

function inflate(var strm: TZStreamRec; flush: Integer): Integer;
begin
   Result:=  SHL_GNU_gzip.inflate(strm,flush);
end;

function inflateEnd(var strm: TZStreamRec): Integer;
begin
   Result:=   SHL_GNU_gzip.inflateEnd(strm);
end;

function deflateInit_(var strm: TZStreamRec; level: Integer; version: PChar;
  recsize: Integer): Integer;
begin
   Result:=   SHL_GNU_gzip.deflateInit_(@strm,level,version,recsize);
end;

function deflate(var strm: TZStreamRec; flush: Integer): Integer;
begin
   Result:=SHL_GNU_gzip.deflate(strm,flush);
end;

function deflateEnd(var strm: TZStreamRec): Integer;
begin
   Result:=SHL_GNU_gzip.deflateEnd(strm);
end;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        {Copies the new data}
        CopyMemory(Pointer({%H-}Longint(Output) + OutputSize - Integer(total_out)),
          @Buffer, total_out);
...
begin
  ...
  {Reads data}
  fYear := ((pByte({%H-}Longint(Data) )^) * 256)+ (pByte({%H-}Longint(Data) + 1)^);
  fMonth := pByte({%H-}Longint(Data) + 2)^;
  fDay := pByte({%H-}Longint(Data) + 3)^;
  fHour := pByte({%H-}Longint(Data) + 4)^;
  fMinute := pByte({%H-}Longint(Data) + 5)^;
  fSecond := pByte({%H-}Longint(Data) + 6)^;
end;
...
begin
  ...
  pByte({%H-}Longint(Data) + 2)^ := Month;
  pByte({%H-}Longint(Data) + 3)^ := Day;
  pByte({%H-}Longint(Data) + 4)^ := Hour;
  pByte({%H-}Longint(Data) + 5)^ := Minute;
  pByte({%H-}Longint(Data) + 6)^ := Second;
...
begin
  ...
  begin
    Output := nil;
    if DecompressZLIB(PChar({%H-}Longint(Data) + Length(fKeyword) + 2),
...
begin
  ...
  begin
    ...
    pByte(Pointer({%H-}Longint(Data) + Length(Keyword) + 1))^ := 0;
    if OutputSize > 0 then
      CopyMemory(Pointer({%H-}Longint(Data) + Length(Keyword) + 2), Output, OutputSize);
...
begin
  ...
  CopyMemory(@fText[1], Pointer({%H-}Longint(Data) + Length(fKeyword) + 1),
    Length(fText));
end;
...
begin
  ...
  if Keyword <> '' then
    CopyMemory(Data, @fKeyword[1], Length(Keyword));
  if Text <> '' then
    CopyMemory(Pointer({%H-}Longint(Data) + Length(Keyword) + 1), @fText[1],
      Length(Text));
  {Let ancestor calculate crc and save}
  Result := inherited SaveToStream(Stream);
end;
...
begin
  ...
  GetPaletteEntries(Source, 0, PaletteSize, Entries{%H-});
...
begin
  ...
  Dest := pChar({%H-}Longint(Dest) + Col * 3);
  repeat
    {Copy this row}
    Byte(Dest^) := fOwner.GammaTable[pByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := fOwner.GammaTable[pByte({%H-}Longint(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := fOwner.GammaTable[pByte({%H-}Longint(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row}
    Byte(Dest^) := Owner.GammaTable[pByte({%H-}Longint(Src) + 4)^]; inc(Dest);
    Byte(Dest^) := Owner.GammaTable[pByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := Owner.GammaTable[pByte({%H-}Longint(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Byte(Extra^) := fOwner.GammaTable[pByte({%H-}Longint(Src) + 5)^]; inc(Extra);
    Byte(Extra^) := fOwner.GammaTable[pByte({%H-}Longint(Src) + 3)^]; inc(Extra);
    Byte(Extra^) := fOwner.GammaTable[pByte({%H-}Longint(Src) + 1)^]; inc(Extra);
    {$ENDIF}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      Dest2 := pChar({%H-}Longint(Dest) + (Header.BitDepth * Col) div 8);
      {Copy data}
...
begin
  ...
  repeat
    ...
    repeat
      {Adjust pointer to pixel byte bounds}
      Dest2 := pChar({%H-}Longint(Dest) + Col div 2);
      {Copy data}
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Dest := pChar({%H-}Longint(Dest) + Col);
  repeat
    {Copy this row}
    Dest^ := Src^; inc(Dest);
    {$IFDEF Store16bits}
    Extra^ := pChar({%H-}Longint(Src) + 1)^; inc(Extra);
    {$ENDIF}
...
begin
  ...
  Col := ColumnStart[Pass];
  Dest := pChar({%H-}Longint(Dest) + Col * 3);
  Trans := pChar({%H-}Longint(Trans) + Col);
  repeat
    {Copy this row and alpha value}
    Trans^ := pChar({%H-}Longint(Src) + 3)^;
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src) + 1)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row and alpha value}
    Trans^ := pChar({%H-}Longint(Src) + 6)^;
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src) + 4)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte({%H-}Longint(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
...
begin
  ...
  begin
    ...
    Data := Pointer({%H-}Longint(Header.ImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow));
    Trans := Pointer({%H-}Longint(Header.ImageAlpha) + ImageWidth * CurrentRow);
    {$IFDEF Store16bits}
    Extra := Pointer({%H-}Longint(Header.ExtraImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow));
    {$ENDIF}
...
begin
  ...
  {%H-}Longint(Data) := {%H-}Longint(Header.ImageData) +
    Header.BytesPerRow * (ImageHeight - 1);
  Trans := Header.ImageAlpha;
  {$IFDEF Store16bits}
  {%H-}Longint(Extra) := {%H-}Longint(Header.ExtraImageData) +
    Header.BytesPerRow * (ImageHeight - 1);
  {$ENDIF}
  {Reads each line}
...
begin
  ...
  begin
    ...
    begin
      {Writes this IDAT chunk}
      WriteIDAT(fStream, Data, Owner.MaxIdatSize - Integer(avail_out));
      {Re-update buffer}
      next_out := Data;
      avail_out := Owner.MaxIdatSize;
    end;

    if Integer(avail_out) < Owner.MaxIdatSize then
      {Writes final IDAT}
      WriteIDAT(fStream, Data, Owner.MaxIdatSize - Integer(avail_out));
...
begin
  ...
  begin
    {Copy pixel values}
    Byte(Dest^) := fOwner.InverseGamma[pByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := fOwner.InverseGamma[pByte({%H-}Longint(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := fOwner.InverseGamma[pByte({%H-}Longint(Src)    )^]; inc(Dest);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
end;
...
begin
  ...
  begin
    ...
    //for sample
    {Copy pixel values}
    pWORD(Dest)^ := fOwner.InverseGamma[pByte({%H-}Longint(Src) + 2)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[pByte({%H-}Longint(Src) + 1)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[pByte({%H-}Longint(Src)    )^]; inc(Dest, 2);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
...
begin
  ...
  begin
    //Now we copy from 1 byte for each sample stored to a 2 bytes (or 1 word)
    //for sample
    pWORD(Dest)^ := pByte({%H-}Longint(Src))^; inc(Dest, 2);
    {Move to next pixel}
    inc(Src);
  end {for I}
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    Byte(Dest^) := Owner.InverseGamma[PByte({%H-}Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := Owner.InverseGamma[PByte({%H-}Longint(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := Owner.InverseGamma[PByte({%H-}Longint(Src)    )^]; inc(Dest);
    Dest^ := Trans^; inc(Dest);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    pWord(Dest)^ := Owner.InverseGamma[PByte({%H-}Longint(Src) + 2)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte({%H-}Longint(Src) + 1)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte({%H-}Longint(Src)    )^]; inc(Dest, 2);
    pWord(Dest)^ := PByte({%H-}Longint(Trans)  )^; inc(Dest, 2);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
...
begin
  ...
  with Header.BitmapInfo.bmiHeader do
    repeat
      ...
      repeat
        {Adjust pointer to pixel byte bounds}
        Src2 := pChar({%H-}Longint(Src) + (biBitCount * Col) div 8);
        {Copy data}
...
begin
  ...
  repeat
    {Copy this row}
    pWord(Dest)^ := pByte({%H-}Longint(Src) + 2)^; inc(Dest, 2);
    pWord(Dest)^ := pByte({%H-}Longint(Src) + 1)^; inc(Dest, 2);
    pWord(Dest)^ := pByte({%H-}Longint(Src)    )^; inc(Dest, 2);
    pWord(Dest)^ := pByte(Trans)^; inc(Dest, 2);
...
begin
  ...
  {%H-}Longint(ImageSource) := {%H-}Longint(Header.ImageData) +
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    24:
      ...
      begin
        ...
        {%H-}Longint(ImageSource) := {%H-}Longint(ImageSourceOrg) - BytesPerRowSrc * j2;
        {%H-}Longint(AlphaSource) := {%H-}Longint(Header.ImageAlpha) +
          BytesPerRowAlpha * j2;
      end;
    ...
    1,4,8: if Header.ColorType = COLOR_GRAYSCALEALPHA then
      ...
      begin
        ...
        {Move pointers}
        {%H-}Longint(ImageData) := {%H-}Longint(ImageData) + BytesPerRowDest;
        if Stretch then j2 := trunc(j / FactorY) else j2 := j;
        {%H-}Longint(ImageSource) := {%H-}Longint(ImageSourceOrg) - BytesPerRowSrc * j2;
        {%H-}Longint(AlphaSource) := {%H-}Longint(Header.ImageAlpha) +
          BytesPerRowAlpha * j2;
...
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
      {%H-}Longint(Result) := {%H-}Longint(ImageAlpha) + (LineIndex * Longint(Width))
...
begin
  with Header do
    {%H-}Longint(Result) := ({%H-}Longint(ExtraImageData) + ((Longint(Height) - 1) *
...
begin
  with Header do
    {%H-}Longint(Result) := ({%H-}Longint(ImageData) + ((Longint(Height) - 1) *
      BytesPerRow)) - (LineIndex * BytesPerRow);
end;
...
begin
  ...
  begin
    ...
    for Line := 0 to Min(CY - 1, Height - 1) do
      CopyMemory(Pointer({%H-}Longint(NewImageData) + (Longint(CY) - 1) *
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer({%H-}Longint(NewImageAlpha) + (Line * CX)),
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer({%H-}Longint(NewImageExtra) + (Line * CX)),
...
begin
  ...
  {Reads data}
  fPPUnitX := ByteSwap(pCardinal({%H-}Longint(Data))^);
  fPPUnitY := ByteSwap(pCardinal({%H-}Longint(Data) + 4)^);
  fUnit := pUnitType({%H-}Longint(Data) + 8)^;
end;
...
begin
  ...
  pCardinal({%H-}Longint(Data) + 4)^ := ByteSwap(fPPUnitY);
  pUnitType({%H-}Longint(Data) + 8)^ := fUnit;
